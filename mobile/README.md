...
- **Con `.env`**: login real (Supabase Auth), varias familias/clientes
  aisladas por RLS, y una misma persona puede tener más de un espacio de
  trabajo (su familia, y aparte sus finanzas personales, por ejemplo) — ver
  `../docs/TESTING_ENVIRONMENT.md` § Nivel 2.
...
## Correr contra tu proyecto real de Supabase (multi-cliente / multi-espacio)

```bash
cp .env.example .env
# completa EXPO_PUBLIC_SUPABASE_URL y EXPO_PUBLIC_SUPABASE_ANON_KEY
npm run web
```

Con eso, la app pide login/registro real; al entrar por primera vez ofrece
**crear un espacio nuevo** (tu familia, o un espacio separado para ti solo)
o **unirte con un código de invitación**. Cada espacio es una fila en
`families`, completamente aislada de las demás por RLS — así es como se
vende a más clientes: cada uno crea el suyo, nadie ve los datos de otro.
Si una persona crea más de un espacio (ej. su familia y, aparte, sus
finanzas personales), puede cambiar entre ellos desde **Más → Cambiar de
espacio**.
...
| Guardado de transacciones, presupuestos, recordatorios | **Real** — AsyncStorage en modo local; Postgres/Supabase en modo remoto |
...
src/domain/store.tsx    store local/demo (AsyncStorage) — modo sin backend
src/domain/supabaseStore.tsx  store real (Postgres) — mismo contexto que store.tsx
src/domain/authStore.tsx      sesión + espacios de trabajo (multi-familia)
src/domain/kipoContext.ts     el contexto que comparten ambos stores
src/screens/            AuthScreen (login/registro), WorkspaceGateScreen (crear/unirse/cambiar)
